...
window.fOneAjax = (c,a,data,succ_callback,dom,hasFile=false)->
    if dom
        k_disable = 0
        $(dom).each(->
...
(->
  ...
  State.forward = (c, a, p) ->
    document.location.href = fGetUrl(c, a, p)

  State.replace = (c, a, p) ->
    document.location.replace fGetUrl(c, a, p)
...
window.fShowHeader = (params)->
    $('.header').replaceWith(headerTpl.render(params));
