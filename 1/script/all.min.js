...
(function(){$(function(){if(SYN_HIGHLIGHT){SyntaxHighlighter.all();}
SHOW_HEADER&&fShowHeader();fShowFooter();fShowCategory();$('.js_logout').click(function(){fOneAjax('Site','AjaxLogout',{},function(o){if(o.code===1){return State.back(0);}},this);return false;});$('.js_to_home').click(function(){State.forward('Main','Index');return false;});return false;});window.fShowHeader=function(){return $('.header').replaceWith(headerTemplate.render(HEADER.params,HEADER.partials));};window.fShowCategory=function(){return $('.category').replaceWith(categoryTemplate.render(CATEGORY.params,CATEGORY.partials));};window.fShowFooter=function(){return $('.footer').replaceWith(footerTemplate.render(FOOTER.params,FOOTER.partials));};window.Pager=function(dom,c,a,params,pager,ajax){var a2,b2,className,has_ellipsis,i,last,link,next,p;if(pager.page_count<=1){return'';}
ajax||(ajax=false);className=ajax?'pager_'+ajax.className:false;last=false;if(pager.page>1){params.p=pager.page-1;last={id:ajax?className+params.p:false,href:fGetUrl(c,a,params)};}
next=false;if(pager.page<pager.page_count){params.p=pager.page+1;next={id:ajax?className+params.p:false,href:fGetUrl(c,a,params)};}
p={className:className,last:last,next:next,list:[]};b2=pager.page-2;a2=pager.page+2;has_ellipsis=false;i=1;while(i<=pager.page_count){if((i>=b2&&i<=a2)||i===1||i===pager.page_count){params.p=i;link={ellipsis:false,id:ajax?className+i:false,href:fGetUrl(c,a,params),name:i,select:i===pager.page};}else{link=has_ellipsis?false:{ellipsis:true};has_ellipsis=true;}
link&&p.list.push(link);i++;}
dom.html(pagerTemplate.render(p));if(ajax){return dom.find('[id^='+className+']').click(function(){p=this.id.replace(className,'');ajax.params.p=p;fOneAjax(ajax.c,ajax.a,ajax.params,ajax.callback,this);return false;});}};window.fGetTime=function(){return Math.floor(STIME+(new Date().getTime()-CTIME)/1000);};window.fGetUrl=function(c,a,p){var arr,i,k,l,param,pieces,url;if(a===undefined){pieces=c.split("/");arr=URLCACHE;i=0;while(i<pieces.length){if(arr[pieces[i]]){arr=arr[pieces[i]];}
i++;}
...
url+='.html';l.length>0&&(url+="?"+l.join("&"));}
return url;}};window.fDateFormat=function(time,flag){var date,ret;date=new Date(time*1000);ret='';if(!flag||flag===1){ret=""+(date.getFullYear())+"-"+(date.getMonth()+1)+"-"+(date.getDate());}else if(flag===2){ret=""+(date.getFullYear())+"-"+(date.getMonth()+1)+"-"+(date.getDate())+" "+(date.getHours())+":"+(date.getMinutes())+":"+(date.getSeconds());}
return ret;};window.fOneAjax=function(c,a,data,succ_callback,dom){var fail,k_disable,succ;if(dom){k_disable=0;$(dom).each(function(){if($(this).data("k_disable")){k_disable=1;}
...
return temstr;};return{set:function(text){return setCookie(uniqueN,text);},clear:function(){return delCookie(uniqueN);},get:function(){return getCookie(uniqueN);}};};window.State={};(function(){State.forwardToUrl=function(url){return document.location.href=url;};State.forward=function(c,a,p){return document.location.href=fGetUrl(c,a,p);};State.replace=function(c,a,p){return document.location.replace(fGetUrl(c,a,p));};return State.back=function(n){n=n||0;return history.go(-n);};})();}).call(this);
(function(){var initializing=false,fnTest=/xyz/.test(function(){xyz})?/\b_super\b/:/.*/;this.Class=function(){};Class.extend=function(prop){var _super=this.prototype;initializing=true;var prototype=new this;initializing=false;for(var name in prop)prototype[name]=typeof prop[name]=="function"&&typeof _super[name]=="function"&&fnTest.test(prop[name])?function(name,fn){return function(){var tmp=this._super;this._super=_super[name];var ret=fn.apply(this,arguments);this._super=tmp;return ret}}(name,prop[name]):prop[name];function Class(){if(!initializing&&this.init)this.init.apply(this,arguments)}
Class.prototype=prototype;Class.prototype.constructor=Class;Class.extend=arguments.callee;return Class}})();var Hogan={};(function(Hogan,useArrayBuffer){Hogan.Template=function(renderFunc,text,compiler,options){this.r=renderFunc||this.r;this.c=compiler;this.options=options;this.text=text||'';this.buf=(useArrayBuffer)?[]:'';}
...
var URLCACHE={"images":{"logo.jpg":"4a5f19fe"}};