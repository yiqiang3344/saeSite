...
var 
    name
,   ss = ''
,   fd = fs.openSync('protected/modules/admin/script/js/main.js', 'a+');
;
helper.scanDir('protected/modules/admin/views/_public/',true).forEach(function(file){
...
helper.rm('protected/modules/admin/script/js/views');
helper.mkdir('protected/modules/admin/script/js/views');
var made_js_map = {};
helper.scanDir('protected/modules/admin/views').forEach(function(dir){
    dirName = helper.baseName(dir);
    if(!helper.isDir(dir) || dirName=='layouts' || dirName=='_public'){
        return;
    }
    //局部子模板
    fs.writeFileSync('protected/modules/admin/script/js/views/'+dirName+'.js', '');
    helper.scanDir(dir).forEach(function(file){
        ...
        fd = fs.openSync('protected/modules/admin/script/js/views/'+dirName+'.js', mode);
        fs.writeSync(fd, ss);
        fs.closeSync(fd);
    });
    var urlDir = 'protected/modules/admin/script/js/views/'+dirName;
    helper.mkdir(urlDir);
